...
export default function Hero() {
  ...
  return (
    ...
    >
      ...
      >
        {/* Badge */}
        <motion.div variants={item} className="mb-6 inline-flex items-center gap-2 rounded-full border border-violet-500/30 bg-violet-500/10 px-4 py-1.5 text-xs font-medium text-violet-300">
          <span className="h-1.5 w-1.5 animate-pulse rounded-full bg-violet-400" />
          Available for opportunities
        </motion.div>

        {/* Heading */}
        <motion.h1 variants={item} className="text-5xl font-extrabold tracking-tight text-white sm:text-6xl lg:text-7xl">
          Hi, I&apos;m{" "}
        ...
        <motion.p variants={item} className="mt-4 text-lg font-medium text-slate-400 sm:text-xl">
          Software Engineering Student
        </motion.p>

        <motion.p variants={item} className="mt-6 max-w-2xl text-base leading-relaxed text-slate-500">
